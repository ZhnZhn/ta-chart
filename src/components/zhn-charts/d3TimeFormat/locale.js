...
, formatDayOfYear = (d, p) => pad(1 + timeDay.count(timeYear(d), d), p, 3)
...
, formatWeekNumberSunday = (d, p) => pad(timeSunday.count(timeYear(d) - 1, d), p, 2)
, dISO = (d) => {
  const day = d.getDay();
  return (day >= 4 || day === 0)
    ? timeThursday(d)
...
, formatWeekNumberMonday = (d, p) => pad(timeMonday.count(timeYear(d) - 1, d), p, 2)
