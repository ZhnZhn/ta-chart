...
describe('d3Color hsl', () => {
  ...
  it("hsl.formatRgb() formats as rgb(…) or rgba(…)", () => {
    expect(hsl("#abcdef").formatRgb()).toBe("rgb(171, 205, 239)");
    expect(hsl("hsl(60, 100%, 20%)").formatRgb()).toBe("rgb(102, 102, 0)");
    expect(hsl("rgba(12%, 34%, 56%, 0.4)").formatRgb()).toBe("rgba(31, 87, 143, 0.4)");
    expect(hsl("hsla(60, 100%, 20%, 0.4)").formatRgb()).toBe("rgba(102, 102, 0, 0.4)");
  });

  it("hsl.formatHsl() formats as hsl(…) or hsla(…)", () => {
    expect(hsl("#abcdef").formatHsl()).toBe("hsl(210, 68%, 80.3921568627451%)");
    expect(hsl("hsl(60, 100%, 20%)").formatHsl()).toBe("hsl(60, 100%, 20%)");
    expect(hsl("rgba(12%, 34%, 56%, 0.4)").formatHsl()).toBe("hsla(210, 64.70588235294117%, 34%, 0.4)");
    expect(hsl("hsla(60, 100%, 20%, 0.4)").formatHsl()).toBe("hsla(60, 100%, 20%, 0.4)");
  });

  it("hsl.formatHsl() clamps to the expected range", () => {
    expect(hsl(180, -100, -50).formatHsl()).toBe("hsl(180, 0%, 0%)");
    expect(hsl(180, 150, 200).formatHsl()).toBe("hsl(180, 100%, 100%)");
    expect(hsl(-90, 50, 50).formatHsl()).toBe("hsl(270, 100%, 100%)");
    expect(hsl(420, 50, 50).formatHsl()).toBe("hsl(60, 100%, 100%)");
  });

  it("hsl.formatHex() formats as #rrggbb", () => {
    expect(hsl("#abcdef").formatHex()).toBe("#abcdef");
    expect(hsl("hsl(60, 100%, 20%)").formatHex()).toBe("#666600");
    expect(hsl("rgba(12%, 34%, 56%, 0.4)").formatHex()).toBe("#1f578f");
    expect(hsl("hsla(60, 100%, 20%, 0.4)").formatHex()).toBe("#666600");
  });
  ...
  it("hsl(h, s, l).clamp() clamps channel values", () => {
    ...
    expect(hsl(-420, -0.1, -0.2, NaN).clamp().opacity).toBe(1);
    expect(hsl(-420, -0.1, -0.2, 0.5).clamp().opacity).toBe(0.5);
    expect(hsl(-420, -0.1, -0.2, -1).clamp().opacity).toBe(0);
    expect(hsl(-420, -0.1, -0.2, 2).clamp().opacity).toBe(1);
  });
  ...
  it("hsl.copy(…) returns a new hsl with the specified channel values", () => {
    const c = hsl(120, 0.3, 0.5, 0.4);
    expect(c.copy() instanceof hsl).toBe(true);
    expect(c.copy().formatHsl()).toBe("hsla(120, 30%, 50%, 0.4)");
    expect(c.copy({opacity: 1}).formatHsl()).toBe("hsl(120, 30%, 50%)");
    expect(c.copy({h: 20}).formatHsl()).toBe("hsla(20, 30%, 50%, 0.4)");
    expect(c.copy({h: 20, s: 0.4}).formatHsl()).toBe("hsla(20, 40%, 50%, 0.4)");
  });
});
