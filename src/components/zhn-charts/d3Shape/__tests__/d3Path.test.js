...
import { assertPathEqual } from "./asserts.test";

describe('d3Path' , () => {
  it("path is an instanceof path", () => {
    const p = path();
    expect(p instanceof path).toBe(true);
    assertPathEqual(p, "");
  });
  ...
  it("path.arc(x, y, radius, startAngle, endAngle) throws an error if the radius is negative", () => {
    ...
    expect(function() { p.arc(100, 100, -50, 0, Math.PI / 2); }).toThrow(/negative radius/);
  });
  ...
  it("path.arcTo(x1, y1, x2, y2, radius) throws an error if the radius is negative", () => {
    const p = path(); p.moveTo(150, 100);
    expect(function() { p.arcTo(270, 39, 163, 100, -53); }).toThrow(/negative radius/);
  });
