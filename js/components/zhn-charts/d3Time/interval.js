...
const timeInterval = (floori, offseti, count, field) => {
  const interval = date => (floori(date = date === void 0 ? new Date() : new Date(+date)), date);
  ...
  interval.offset = (date, step) => (offseti(date = new Date(+date), step == null ? 1 : mathFloor(step)), date);
