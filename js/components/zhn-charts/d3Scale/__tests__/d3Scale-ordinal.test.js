...
describe('d3Scale ordinal', () => {
  ...
  it("ordinal(x) implicitly extends the domain when a range is explicitly specified", () => {
    const s = (0, _index.scaleOrdinal)().range(["foo", "bar"]);
    expect(s.domain()).toEqual([]);
    expect(s(0)).toBe("foo");
    expect(s.domain()).toEqual([0]);
    expect(s(1)).toBe("bar");
  ...
  it("ordinal.domain() uniqueness is based on primitive coercion", () => {
    const s = (0, _index.scaleOrdinal)().domain(["foo"]).range([1, 2, 3]);
  ...
  it("ordinal.domain() does not coerce domain values to strings", () => {
    const s = (0, _index.scaleOrdinal)().domain([0, 1]);
    expect(s.domain()).toEqual([0, 1]);
  ...
  it("ordinal.domain() does not barf on object built-ins", () => {
    const s = (0, _index.scaleOrdinal)().domain(["__proto__", "hasOwnProperty"]).range([1, 2]);
  ...
  it("ordinal.domain() accepts dates", () => {
    const s = (0, _index.scaleOrdinal)().domain([new Date(1970, 2, 1), new Date(2001, 4, 13), new Date(1970, 2, 1), new Date(2001, 4, 13)]);
    s(new Date(1970, 2, 1));
  ...
  it("ordinal.range(x) makes a copy of the range", () => {
    ...
    const s = (0, _index.scaleOrdinal)().range(range);
    range.push("blue");
    expect(s.range()).toEqual(["red", "green"]);
  });
  it("ordinal.range() accepts an iterable", () => {
    const s = (0, _index.scaleOrdinal)().range(new Set(["red", "green"]));
    expect(s.range()).toEqual(["red", "green"]);
  });
  it("ordinal.range() returns a copy of the range", () => {
    const s = (0, _index.scaleOrdinal)().range(["red", "green"]);
    const range = s.range();
  ...
  it("ordinal(value) recycles values when exhausted", () => {
    const s = (0, _index.scaleOrdinal)().range(["a", "b", "c"]);
    expect(s(0)).toBe("a");
    expect(s(1)).toBe("b");
    expect(s(2)).toBe("c");
    expect(s(3)).toBe("a");
  ...
  it("ordinal.unknown(x) sets the output value for unknown inputs", () => {
    const s = (0, _index.scaleOrdinal)().domain(["foo", "bar"]).unknown("gray").range(["red", "blue"]);
  ...
  it("ordinal.unknown(x) prevents implicit domain extension if x is not implicit", () => {
    const s = (0, _index.scaleOrdinal)().domain(["foo", "bar"]).unknown(undefined).range(["red", "blue"]);
    expect(s("baz")).toBe(undefined);
    expect(s.domain()).toEqual(["foo", "bar"]);
  });
  it("ordinal.copy() copies all fields", () => {
    const s1 = (0, _index.scaleOrdinal)().domain([1, 2]).range(["red", "green"]).unknown("gray");
    const s2 = s1.copy();
  ...
  it("ordinal.copy() changes to the domain are isolated", () => {
    const s1 = (0, _index.scaleOrdinal)().range(["foo", "bar"]);
    const s2 = s1.copy();
    s1.domain([1, 2]);
...
//# sourceMappingURL=d3Scale-ordinal.test.js.map