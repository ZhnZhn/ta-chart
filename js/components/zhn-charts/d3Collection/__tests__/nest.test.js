...
describe('d3Collection nest', () => {
  it("nest.entries(array) returns the array of input values, in input order", () => {
    expect((0, _index.nest)().entries([1, 2, 3])).toEqual([1, 2, 3]);
    expect((0, _index.nest)().entries([1, 3, 2])).toEqual([1, 3, 2]);
    expect((0, _index.nest)().entries([3, 1, 2])).toEqual([3, 1, 2]);
  });
  it("nest.sortValues(order).entries(array) returns input values in sorted order", () => {
    const nestAscending = (0, _index.nest)().sortValues((a, b) => a.foo - b.foo),
      nestDescending = (0, _index.nest)().sortValues((a, b) => b.foo - a.foo),
      a = {
        foo: 1
      },
      b = {
  ...
  it("nest.key(key).entries(array) returns entries for each distinct key, with values in input order", () => {
    const _nest = (0, _index.nest)().key(d => d.foo).sortKeys(_d3Array.ascending),
      a = {
        foo: 1
      },
      b = {
    ...
    expect(_nest.entries([c, a, b, c])).toEqual([{
      key: "1",
      values: [a, b]
    }, {
      key: "2",
    ...
    expect(_nest.entries([c, b, a, c])).toEqual([{
      key: "1",
      values: [b, a]
    }, {
      key: "2",
  ...
  it("nest.key(key) coerces key values to strings", () => {
    const _nest = (0, _index.nest)().key(d => d.number ? 1 : "1").sortKeys(_d3Array.ascending),
      a = {
        number: true
      },
      b = {
    ...
    expect(_nest.entries([a, b])).toEqual([{
      key: "1",
      values: [a, b]
    }]);
  });
  it("nest.key(key1).key(key2).entries(array) returns entries for each distinct key set, with values in input order", () => {
    const _nest = (0, _index.nest)().key(d => d.foo).sortKeys(_d3Array.ascending).key(d => d.bar).sortKeys(_d3Array.ascending),
      a = {
        foo: 1,
        bar: "a"
      },
    ...
    expect(_nest.entries([a, b, c, d, e, f])).toEqual([{
      key: "1",
      values: [{
        key: "a",
        values: [a, b]
    ...
    expect(_nest.entries([f, e, d, c, b, a])).toEqual([{
      key: "1",
      values: [{
        key: "a",
        values: [b, a]
